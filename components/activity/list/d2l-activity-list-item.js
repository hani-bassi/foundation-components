...
class ActivityListItem extends HypermediaStateMixin(ListItemAccumulatorMixin(LitElement)) {
	...
	render() {
		return this._renderListItem({
			illustration: html`${guard([this._activityHref, this.token], () => html`<d2l-activity-image class="d2l-activity-list-item-image" href="${this._activityHref}" .token="${this.token}"></d2l-activity-image>`)}`,
